...
- Replace `{sdk}` with the absolute path to the `env.sh` in the root directory of the [ameba-rtos SDK](https://github.com/Ameba-AIoT/ameba-rtos).  
...
In the demo root directory (e.g., this example directory or `HELLO_WORLD`), run:
...
> Adjust the paths if you want to use bin files from the parent directory or other locations.
